...
struct GitHubRestAPI {
    ...
    func dispatchableWorkflows(owner: String, name: String, limit: Int = 50) async throws -> [RepoWorkflowSummary] {
        let token = try await tokenProvider()
        let baseURL = await apiHost()
        var components = URLComponents(
            url: baseURL.appending(path: "/repos/\(owner)/\(name)/actions/workflows"),
            resolvingAgainstBaseURL: false
        )!
        components.queryItems = [URLQueryItem(name: "per_page", value: "\(max(1, min(limit, 100)))")]
        let (data, _) = try await authorizedGet(url: components.url!, token: token)
        let response = try GitHubDecoding.decode(ActionsWorkflowsResponse.self, from: data)
        var workflows: [RepoWorkflowSummary] = []

        for workflow in response.workflows where workflow.state != "disabled_manually" {
            guard let raw = try? await self.repoFileContents(owner: owner, name: name, path: workflow.path),
                  let inputs = WorkflowDispatchInputParser.parse(raw)
            else { continue }

            workflows.append(RepoWorkflowSummary(
                id: workflow.id,
                name: workflow.name,
                path: workflow.path,
    ...
    func recentWorkflowRuns(
        ...
        return try await self.recentList(
            owner: owner,
            name: name,
