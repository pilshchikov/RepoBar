...
This fork keeps the upstream RepoBar app, plus local changes that make it behave more like a persistent GitHub work dashboard.
...
- Running workflows stays in GitHub itself. RepoBar keeps this menu focused on finding the workflow and tracking recent history.
...
The menu-stability and workflow-history changes were replayed on top of that fork instead of replacing those refactors.
...
- `WorkflowDispatchInputParser.swift` intentionally handles enough of the common workflow YAML shape to detect `workflow_dispatch` without adding a new dependency.
